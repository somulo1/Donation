  ...
  try {
    ...
    if (donation.status === 'pending') {
      ...
      if (timeDiff > 300000) { // 5 minutes timeout
        ...
        const remainingTime = Math.max(0, 300 - Math.floor(timeDiff / 1000)); // 5 minutes
        return json({
          success: true,
          status: 'pending',
